...
class FaceService:
    ...
    def crop_primary_face(self, cv_image: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[Tuple[int, int, int, int]]]:
        ...
        if not boxes and w > 300 and h > 200:
            left_w = int(w * 0.45)
            left_sub = cv_image[:, :left_w]
            left_boxes = self.detect_faces(left_sub, conf_threshold=0.35)
            if left_boxes:
                boxes = left_boxes  # coordinates are already relative to (0, 0)
            else:
    ...
    def verify_faces(self, doc_cv: np.ndarray, person_cv: Optional[np.ndarray]) -> FaceVerificationResult:
        doc_crop, doc_box = self.crop_primary_face(doc_cv)
        ...
        person_crop, person_box = self.crop_primary_face(person_cv)
